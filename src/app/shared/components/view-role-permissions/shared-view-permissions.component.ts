...
export class SharedRoleViewPermissionsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.ngxPermission
      .hasPermission(['root_admin', 'view_role_permissions'])
      .then((has) => {
        if (has) this.store.dispatch(fetchRolePermissions({ id: this.rowId }));
      });
